...
if (-not (Get-Command ssh -ErrorAction SilentlyContinue)) { throw "OpenSSH client not found on this PC." }
...
set -e
cd ~
for s in $(systemctl list-unit-files | awk '/^rdwc.*\.service/ {print $1}'); do
  sudo systemctl stop "$s" || true
done
STAMP=$(date +%Y%m%d_%H%M%S)
RDWC_DIRS=$(ls -d RDWC* rdwc* 2>/dev/null || true)
if [ -n "$RDWC_DIRS" ]; then
  tar -czf "RDWC-backup_${STAMP}.tar.gz" $RDWC_DIRS
  echo "BACKUP=RDWC-backup_${STAMP}.tar.gz"
else
  echo "BACKUP="
fi
'@
Invoke-SSH $bashScript1
...
set -e
cd ~
rm -rf ~/RDWC-v3 ~/RDWC-v2 ~/RDWC-v1 ~/RDWC ~/rdwc ~/RDWC.v3 || true
for f in /etc/systemd/system/rdwc*.service; do
  [ -e "$f" ] && sudo rm "$f"
done
sudo systemctl daemon-reload
sudo apt-get update -y
sudo apt-get install -y i2c-tools python3-venv python3-pip
sudo raspi-config nonint do_i2c 0
sudo reboot
'@
Invoke-SSH $bashScript3
...
for ($i=1; $i -le $maxTries; $i++) {
  try {
    ...
    if ($i -eq $maxTries) { throw "Pi did not come back on SSH in time." }
  }
}

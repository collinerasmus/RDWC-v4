...
function RunSSH($cmd){ & ssh -p $Port "${PiUser}@${PiHost}" $cmd }
function RunSCP($src,$dst){ & scp -P $Port $src "${PiUser}@${PiHost}:${dst}" }
...
RunSSH "set -e; sudo apt-get update -y; sudo apt-get install -y mjpg-streamer; sudo tee /etc/systemd/system/mjpg-streamer.service >/dev/null <<'UNIT'
[Unit]

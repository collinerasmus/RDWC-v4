...
def test_calibrate_ec_dry_default_value(mock_i2c_available, mock_settings, mock_ezo_device):
    ...
    with patch('app.sensor_controller.ezo_i2c_stabilized.EZO', return_value=mock_ezo_device), \
        ...
        # Verify Cal,dry command was sent
...
def test_calibrate_ec_low_default_is_84(mock_i2c_available, mock_settings, mock_ezo_device):
    ...
    with patch('app.sensor_controller.ezo_i2c_stabilized.EZO', return_value=mock_ezo_device), \
        ...
        assert result["ok"] is True
        # Verify Cal,low,84 command was sent
        calls = [str(call) for call in mock_ezo_device.cmd.call_args_list]
...
def test_calibrate_ec_low_custom_value(mock_i2c_available, mock_settings, mock_ezo_device):
    ...
    with patch('app.sensor_controller.ezo_i2c_stabilized.EZO', return_value=mock_ezo_device), \
        ...
        assert result["ok"] is True
        # Verify Cal,low,1413 command was sent
        calls = [str(call) for call in mock_ezo_device.cmd.call_args_list]
        assert any("Cal,low,1413" in str(call) for call in calls)


def test_get_ec_calibration_status_parses_dry(mock_i2c_available, mock_settings, mock_ezo_device):
    """Test calibration status correctly parses dry calibration."""
    # Mock response indicating dry+two-point calibration
    mock_ezo_device.cmd = Mock(return_value="?CAL,3")
    ...
    with patch('app.sensor_controller.ezo_i2c_stabilized.EZO', return_value=mock_ezo_device), \
        ...
        assert result["cal"] == "dry+two-point"


def test_get_ec_calibration_status_parses_one_point(mock_i2c_available, mock_settings, mock_ezo_device):
    """Test calibration status correctly parses one-point calibration."""
    # Mock response indicating one-point calibration
    ...
    with patch('app.sensor_controller.ezo_i2c_stabilized.EZO', return_value=mock_ezo_device), \
        ...
        assert result["cal"] == "one-point"


def test_get_ec_calibration_status_parses_uncalibrated(mock_i2c_available, mock_settings, mock_ezo_device):
    """Test calibration status correctly parses uncalibrated state."""
    # Mock response indicating uncalibrated
